...
''',
    ...
    group('Environment Configuration Tests', () {
      test('should load PROD_SERVER_BASEURL correctly', () {
        expect(PROD_SERVER_BASEURL, equals('https://api.exchek.com'));
      });

      test('should create EnvConfig with prod base URL', () {
        final prodConfig = EnvConfig(baseUrl: PROD_SERVER_BASEURL);
        expect(prodConfig.baseUrl, equals('https://api.exchek.com'));
      });

      test('should initialize FlavorConfig with prod flavor', () {
        final prodConfig = EnvConfig(baseUrl: PROD_SERVER_BASEURL);
        FlavorConfig.initialize(flavor: Flavor.prod, env: prodConfig);

        expect(FlavorConfig.instance.flavor, equals(Flavor.prod));
        expect(FlavorConfig.instance.env.baseUrl, equals('https://api.exchek.com'));
    ...
    group('Main Function Components Integration', () {
      test('should initialize all components in correct order for production', () async {
        // Test the sequence of operations that happen in main()

        // 1. Load environment variables - use test values
        const testBaseUrl = 'https://api.exchek.com';

        // 2. Create environment config
        final prodConfig = EnvConfig(baseUrl: testBaseUrl);
        expect(prodConfig.baseUrl, equals(testBaseUrl));
